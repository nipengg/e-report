...
const ShowClass = () => {
    ...
    const getData = async () => {
        const response = await axios.get(`${url}course/attend/${courseID}`, {
            headers: {
        ...
        <>
            ...
            <Container style={{ height: "108vh" }}>
                {loading === true ? <div style={{ textAlign: 'center', paddingTop: 250 }}><CDBSpinner dark /></div> :
                    <>
                        <Row>
                            <Col sm={11}><h1>{courseName[0].course_name} - Class</h1></Col>
                            ...
                            <tbody>
                                {data.map((item, index) => {
                                    return (
                                        <tr key={index}>
                                            <td>{item.class_id}</td>
                                            <td><a href={'/course/' + courseID + '/' + item.class_id}> {item.class_name}</a></td>
